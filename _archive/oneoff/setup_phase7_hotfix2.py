# -*- coding: utf-8 -*-
r'''
setup_phase7_hotfix2.py - 突触传导动效增强
光点加大+光晕+拖尾(彗星轨迹)+缓动+时长900ms+错峰120ms
仅 render.js / main.js; 测试不受影响
'''
import sys
...
FILES["web/js/main.js"] = r'''(function () {
  ...
  function openDetail(key) {
    ...
    var src = n.source === "pep" ? "人教初中 · " + n.book
      : n.source === "marble" ? "Marble 小学" : "K12 数据集";
    var st = statusOf(key);
...
def main():
    ...
    print("[1/2] 自检: " + ("[PASS] 无污染" if not bad else "[FAIL] " + ", ".join(bad)))
    if bad:
        return
    for rel, content in FILES.items():
        ...
        print("  [OVERWRITE] " + rel)
    print("[2/2] 完成: 拖尾光点(彗星轨迹+光晕+缓动+900ms+120ms错峰)")
    print("=" * 56)
    print("验收: 重启服务, Ctrl+F5(建议无痕窗口防缓存),")
    print("  恢复快照到近全灰 -> 掌握一个根节点 -> 看彗星拖尾飞向后代")
    print("  拍视频: 给节点特写 -> 飞行瞬间 -> 拉远看点亮一片")
