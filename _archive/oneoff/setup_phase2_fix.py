# -*- coding: utf-8 -*-
r'''setup_phase2_fix.py - 阶段2 runner v2: 同指规则 + 相似度地板0.35
    1) 归档 pilot map -> phase2_map.pilot1.jsonl (v2 重跑不跳过)
    2) 重写 run_phase2.py  3) 语法校验'''
import shutil
from pathlib import Path

ROOT = Path(__file__).resolve().parent
MAP = ROOT / "data" / "phase2_map.jsonl"
RUN = ROOT / "run_phase2.py"

if MAP.exists():
    bak = MAP.with_name("phase2_map.pilot1.jsonl")
    shutil.move(str(MAP), str(bak))
    print("[OK] pilot map 已归档 -> " + bak.name)
else:
    print("[SKIP] 无 map 文件")
...
if "PHASE2_RUNNER_V2" in RUN.read_text(encoding="utf-8"):
    print("[SKIP] runner 已是 v2")
else:
    compile(SRC, str(RUN), "exec")
    RUN.write_text(SRC, encoding="utf-8")
    print("[OK] run_phase2.py 已升级 v2 (同指规则 + 相似度地板0.35)")

print("下一步: 重跑pilot -> 重审")
