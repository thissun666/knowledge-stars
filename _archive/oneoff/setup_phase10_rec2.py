# -*- coding: utf-8 -*-
r'''setup_phase10_rec2.py - 推荐路径改造 v2(正则定位, 不依赖函数顺序)
    后端: recommend_paths -> 每科一条/学习中锚定/完成才更换 | k上限 40
    前端: 弹窗全科目 + 文案 + 继续学习区分 | v=12 | 新增 3 测试'''
import re
...
def clean(piece: str) -> bool:
    return BAD not in piece and not any(x in piece for x in CURLY)
...
def main():
    ...
    # 1. graph_service: 正则定位整个 recommend_paths 函数(与侦察脚本同款)
    gs_p = ROOT / "backend" / "graph_service.py"
    src = gs_p.read_text(encoding="utf-8")
    m = re.search(r"    def recommend_paths\(self.*?(?=\n    def |\nclass |\Z)",
                  src, re.S)
    if not m:
        print("[FAIL] graph_service 未匹配到 recommend_paths, 中止")
        ok = False
    if ok and not clean(NEW_FN):
        print("[FAIL] 注入内容自检未过, 中止")
        ok = False
    if ok:
        cand = src[:m.start()] + NEW_FN + "\n" + src[m.end():]
        try:
            compile(cand, "graph_service.py", "exec")
        except SyntaxError as e:
            print("[FAIL] 改写后语法校验失败: " + str(e))
            ok = False
    if ok:
        gs_p.write_text(cand, encoding="utf-8")
        print("[OK] recommend_paths -> 每科一条/学习中锚定/完成才更换 (已编译校验)")
    ...
    if ok:
        ...
        if "min(40, k)" in rt:
            print("[SKIP] 路由 k 上限已是 40")
        elif "min(10, k)" in rt:
            rt = rt.replace("min(10, k)", "min(40, k)")
            rt_p.write_text(rt, encoding="utf-8")
            print("[OK] 路由 k 上限 10 -> 40")
        else:
            print("[WARN] 路由 k 上限锚点未找到(功能可用, 仅上限旧值)")
    ...
    if ok:
        ...
        if n:
            ...
            print("[OK] 弹窗拉全科目 (替换 " + str(n) + " 处)")
        elif "API.recommend(30, currentBand)" in mj:
            print("[SKIP] 弹窗已拉全科目")
        else:
            print("[WARN] 未找到推荐调用锚点, 回报我")
        if "由图谱算法直出(解锁杠杆排序)" in mj:
            mj = mj.replace("由图谱算法直出(解锁杠杆排序)",
                            "由图谱算法直出(每科一条·完成才更换)")
            print("[OK] meta 文案已更新")
        elif "每科一条" in mj:
            print("[SKIP] meta 文案已更新过")
        else:
        ...
        if old_toast in mj:
            ...
            print("[OK] toast 区分 锚定/新推荐")
        elif "继续学习: " in mj:
            print("[SKIP] toast 已区分")
        else:
    ...
    if ok:
        ...
        if "(k || 30)" in ap:
            print("[SKIP] api.js 默认 k 已是 30")
        elif "(k || 5)" in ap:
            ap = ap.replace("(k || 5)", "(k || 30)")
            ap_p.write_text(ap, encoding="utf-8")
            print("[OK] api.js 默认 k -> 30")
        else:
            print("[WARN] api.js 锚点未找到")

    # 5. 测试文件 + 编译校验
    if ok:
        try:
            compile(TESTS, "test_recommend_per_subject.py", "exec")
        except SyntaxError as e:
            print("[FAIL] 测试文件语法校验失败: " + str(e))
            ok = False
        if ok:
    ...
    if ok:
        ...
        print("  重启服务+刷新(v=12), 三学段各开一次推荐路径:")
        print("    小学 3 条 | 初中 10 条 | 高中 6 条(含数学)")
        print("    标学习中 -> 该科置顶锁定; 掌握它 -> 再开换下一条")
        print("=" * 56)
